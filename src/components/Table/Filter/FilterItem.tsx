...
const FilterItem = (props: RowFilterProps) => {
    ...
    const filterColumn = columns.find(
        item =>
            (Array.isArray(item.dataIndex)
                ? item.dataIndex.join('.')
                : item.dataIndex) === currentFilter.field
    );
    ...
    const renderDataComponent = () => {
        switch (filterType) {
            ...
            case 'number':
                return <InputNumber style={{ width: '100%' }} placeholder={intl.formatMessage({ id: 'global.table.customfilter.label.giatri' })} />;
            case 'select':
                return (
                    <Select
                            ...
                            <Form.Item
                                ...
                                label={<Space>
                                    <Form.Item valuePropName="checked"
                                        initialValue={true} name={[...namePath, 'active']} noStyle>
                        ...
                        {!!currentFilter.operator &&
                            ...
                            <>
                                <Col
                                    span={24}
                                    md={
                                        currentFilter.operator === EOperatorType.BETWEEN || currentFilter.operator === EOperatorType.NOT_BETWEEN ? 12 : 24
                                    }
                                >
                                    <Form.Item
                                        name={
                                            currentFilter.operator === EOperatorType.INCLUDE || currentFilter.operator === EOperatorType.NOT_INCLUDE
                                                ? [...namePath, 'values']
                                                : [...namePath, 'values', 0]
                                        }
                                        rules={[...rules.required]}
                                ...
                                {currentFilter.operator === EOperatorType.BETWEEN || currentFilter.operator === EOperatorType.NOT_BETWEEN ? (
                                    <Col span={24} md={12}>
                                        <Form.Item name={[...namePath, 'values', 1]} rules={[...rules.required]} label={intl.formatMessage({ id: 'global.table.customfilter.label.giatriden' })}>
                                            {renderDataComponent()}
                                        </Form.Item>
                                    </Col>
                                ) : null}
