import { ELoaiToChucDayHoc } from '@/services/DanhMucHeThong/constant';
...
const FormLichTrinhCuThe = (props: { initWeek?: number; getData: any }) => {
  ...
  const {
    ...
    putModel,
    formSubmiting,
    putOrPostNoiDungModel,
  } = useModel('hocphan.tientrinhhp');
  ...
  useEffect(() => {
    if (record?._id) {
      ...
      form.resetFields();
      if (initWeek) form.setFieldsValue({ tuan: initWeek });
    }
  }, [record?._id, initWeek]);
  ...
  const onFinish = async (values: any) => {
    ...
    if (edit) {
      ...
      Object.values(ELoaiToChucDayHoc).forEach((loai) =>
      ...
      <Row gutter={[12, 0]} style={{ marginBottom: 12 }}>
        <Col span={24} md={6}>
          <Form.Item
            name="tuan"
            label="Tuần học"
            rules={[...rules.required, ...rules.number(53, 1, false)]}
          >
            <InputNumber
