...
import TrongSoHocPhanPage from '../TrongSoHocPhan';
import GiangVienDeCuongPage from '../GiangVienDeCuong';

const ModalDeCuongHocPhan = (props: any) => {
  const { record, edit, setVisibleForm } = useModel('hocphan.decuonghocphan');
  ...
  return (
    <Card title={`${edit ? 'Chỉnh sửa' : 'Thêm mới'} ${title?.toLowerCase()} ${record?.ma ?? ''}`}>
      ...
      >
        ...
        {/* <Steps.Step title="Lịch trình chung" disabled={!record?._id} /> */}
        <Steps.Step title="Lịch trình cụ thể" disabled={!record?._id} />
        <Steps.Step title="Học liệu" disabled={!record?._id} />
        <Steps.Step title="Đánh giá KQHT" disabled={!record?._id} />
      </Steps>

      {currentStep === 0 ? (
        <FormDeCuong afterAddNew={() => setCurrentStep(1)} />
      ) : currentStep === 1 ? (
