...
import FormAttributesCompetency from './Form';

const FormItemAttributesCompetency = (props: {
	value?: Competency.ICompetencyAttributes[];
	onChange?: (data: Competency.ICompetencyAttributes[]) => void;
	disabled?: boolean;
}) => {
	const intl = useIntl();
	const { setVisibleForm, visibleForm, setEdit, edit, setRecord, setIsView } = useModel('danhmuc.competencyattributes');
	...
	const onAdd = (records: Competency.ICompetencyAttributes[]) => {
		const data = [...value];
		...
		<>
			<TableStaticData data={value} columns={columns} size='small' hasTotal addStt>
				{!disabled && (
					<Button
						onClick={() => {
							setRecord({} as Competency.ICompetencyAttributes);
							setEdit(false);
							setIsView(false);
							setVisibleForm(true);
						}}
			...
			<Modal
				title={edit ? 'Edit competency attribute' : 'Add new competency attribute'}
				open={visibleForm}
				width={600}
				footer={null}
				onCancel={() => setVisibleForm(false)}
			>
				<FormAttributesCompetency onOk={onAdd} />
			</Modal>
		</>
	);
};

export default FormItemAttributesCompetency;
