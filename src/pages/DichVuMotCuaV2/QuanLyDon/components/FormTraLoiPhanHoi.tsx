...
const FormTraLoiPhanHoi = () => {
  const [form] = Form.useForm();

  const { loading, record, setVisibleForm, traLoiPhanHoiDvmcModel } = useModel('phanhoi');

  const { recordDon, chuyenVienDieuPhoiGetDonModel } = useModel('dichvumotcuav2');
  return (
    <Card loading={loading} title="Trả lời phản hồi">
      <Form
        labelCol={{ span: 24 }}
        onFinish={async (values: PhanHoi.IRecord) => {
          traLoiPhanHoiDvmcModel(recordDon?._id ?? '', values, () => {
