...
const ModalImport = (props: {
	...
	const { danhSach, setDanhSach } = useModel(
		role === EVaiTroKhaoSat.SINH_VIEN ? 'thongbao.sinhvien' : 'thongbao.nhansu',
	) as any;
	...
	const onFinish = async (values: any) => {
		...
		if (customImport?.onImport) {
			...
			try {
				...
				if (setDanhSach) {
					setDanhSach(_.uniqBy([...(danhSach || []), ...res], (item) => item.username || item.code));
				}
